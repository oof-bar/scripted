...
// Returns an instance of U for OO-style calls
function U($item=null) {
  $U = new U;
  if(func_num_args() > 0) $U->_wrapped = $item;
  return $U;
}
...
class U {
  ...
  public function chain($item=null) {
    ...
    $U = (isset($this) && isset($this->_chained) && $this->_chained) ? $this : U($item);
    $U->_chained = true;
    return $U;
  }
  ...
  public function reduceRight($collection=null, $iterator=null, $memo=null) {
    ...
    $U = new self;
    return self::_wrap($U->reduce($collection, $iterator, $memo));
  }
  ...
  public function invoke($collection=null, $function_name=null, $arguments=null) {
    ...
    $U = new self;
    list($collection, $function_name) = $U->first($args, 2);
    $arguments = $U->rest(func_get_args(), 2);
  ...
  public function any($collection=null, $iterator=null) {
    ...
    $U = new self;
    if(!is_null($iterator)) $collection = $U->map($collection, $iterator);
  ...
  public function initial($collection=null, $n=null) {
    ...
    $U = new self;
    return self::_wrap($U->first($collection, $first_index));
  }
  ...
  public function compact($collection=null) {
    ...
    $U = new self;
    return self::_wrap($U->select($collection, function($val) {
      return (bool) $val;
    }));
  }
  ...
  public function flatten($collection=null, $shallow=null) {
    ...
    if(count($collection) > 0) {
      foreach($collection as $item) {
        if(is_array($item)) {
          $U = new self;
          $return = array_merge($return, ($shallow) ? $item : $U->flatten($item));
        }
        else $return[] = $item;
      }
    }
  ...
  public function intersection($array=null) {
    ...
    $U = new self;
    $return = $U->first($arrays);
    foreach($U->rest($arrays) as $next) {
      if(!$U->isArray($next)) $next = str_split((string) $next);
  ...
  public function union($array=null) {
    ...
    $U = new self;
    return self::_wrap($U->flatten(array_values(array_unique(call_user_func_array('array_merge', $arrays)))));
  }
  ...
  public function lastIndexOf($collection=null, $item=null) {
    ...
    krsort($collection);
    $U = new self;
    return self::_wrap($U->indexOf($collection, $item));
  }
  ...
  public function range($stop=null) {
    ...
    $U = new self;
    $args = $U->reject($args, function($val) {
      return is_null($val);
    });
    ...
    if($step > 0 && $U->last($results) >= $stop) array_pop($results);
    elseif($step < 0 && $U->last($results) <= $stop) array_pop($results);
  ...
  public function zip($array=null) {
    ...
    $U = new self;
    $num_return_arrays = $U->max($U->map($arrays, function($array) {
      return count($array);
    }));
    $return_arrays = $U->range($num_return_arrays);
  ...
  public function max($collection=null, $iterator=null) {
    ...
    arsort($results);
    $U = new self;
    $first_key = $U->first(array_keys($results));
  ...
  public function extend($object=null) {
    ...
    $array = (array) $object;
    $U = new self;
    $extensions = $U->rest(func_get_args());
  ...
  public function isEqual($a=null, $b=null) {
    ...
    if(is_object($a) || is_array($a)) {
      ...
      $U = new self;
      $keys_equal = $U->isEqual($U->keys($a), $U->keys($b));
      $values_equal = $U->isEqual($U->values($a), $U->values($b));
